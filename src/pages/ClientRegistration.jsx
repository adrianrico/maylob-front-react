...
export default function ClienRegistration()
{
 ...
 //#region   [ DATA LOAD ] 
  ...
  useEffect(() =>
  {
    ...
    {  
      switch (true) 
      {
        case (!res.ok || res.status == 500):
          ...
          throw new Error('Problema de conexión con el servidor... [38]')
        break;
        ...
        default:
          throw new Error('Connection established: '+res.ok+'\nConnection STATUS: '+res.status)
        break;
      }
    })
    .then(data => 
    ...
    .catch(err => 
      { 
        console.log(err) 
        openModal(['No hay conexión con el servidor ⛟', 'Por favor inténtalo más tarde ⛟'],0 )
      })
  },[]) 
...
//#region    [ LOGIC AND STATES ]
    ...
    function handleChange(event)
    {
      ...
      setFormValues({...newSelectedObject,client_search:newSelectedObject.client_name})
    }
          ...
          case 200:
            ...
            {  
              switch (true) 
              {
                case (!res.ok && res.status == 500):
                  setFormDisplay(false)
                  toast.error('No hay conexión con el servidor',{duration:4000,position:'bot-center'});
